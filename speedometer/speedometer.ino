...
#define MAXSPEED 4500 //mm/s (4500mm/s ~10mph)
...
#define NUMSENSORS 1 //i.e., if you may have 2 wheel magnets on a magnetic wheel sensor.  The more you add, the better your resolution will be!
const double DISTANCE_PER_PULSE = (double) WHEELDIA*3.1415926/NUMSENSORS; //mm
#define SPEEDBUFFSIZE 5 //smoothing constant. larger:less-responsive, smaller:more-responsive. Recommend 3-10
...
void setup() {
  ...
  Serial.println(DISTANCE_PER_PULSE);
 ...
 */
void moveDial() {
   double approxDestStep = 0.0;
   double currSpeed = 0.0;
   for (int i = 0; i<SPEEDBUFFSIZE; ++i) {
     currSpeed = speedCounts[i]*DISTANCE_PER_PULSE/(T_INTERVAL/1000.0); //distance/time = speed 
     //Serial.print("curr speed: "); Serial.println(currSpeed);
     if (currSpeed >= MAXSPEED) {
       approxDestStep += (MAXSPEEDANGLE/360.0)*STEPSPERREV/SPEEDBUFFSIZE;
       Serial.println("Max speed hit!");
     }
     else {
       approxDestStep += map(currSpeed, 
                              ...
                              MAXSPEED, 
                              MINSPEEDANGLE, 
                              (MAXSPEEDANGLE/360.0)*STEPSPERREV)
